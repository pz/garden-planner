 ...
 * it's a reasonable starting point, not a substitute for a real zone lookup.
 */
...
export type GeoZoneStatus = 'idle' | 'locating' | 'done' | 'denied' | 'unsupported' | 'error';

export function detectZoneFromLocation(): Promise<{ zoneId: string; latitude: number; longitude: number }> {
  return new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        resolve({ zoneId: zoneIdFromLatitude(latitude), latitude, longitude });
      },
      (err) => {
        reject(err);
      },
